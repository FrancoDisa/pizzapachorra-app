...
export class ClientesModel {
   ...
   */
  async getAll(limit?: number, offset?: number): Promise<Cliente[]> {
    try {
   ...
   */
  async getByTelefono(telefono: string): Promise<Cliente | null> {
    try {
   ...
   * Crear nuevo cliente
   */
  async create(clienteData: Omit<Cliente, 'id' | 'total_pedidos' | 'total_gastado' | 'ultimo_pedido' | 'created_at' | 'updated_at'>): Promise<Cliente> {
    try {
      const query = `
   ...
   */
  async update(id: number, clienteData: Partial<Omit<Cliente, 'id' | 'total_pedidos' | 'total_gastado' | 'ultimo_pedido' | 'created_at' | 'updated_at'>>): Promise<Cliente | null> {
    try {
      const updateFields = [];
      const values = [];
   ...
   */
  async autocompleteByTelefono(query: string, limit = 10): Promise<Pick<Cliente, 'telefono' | 'nombre' | 'direccion'>[]> {
    try {
      const searchQuery = `
   ...
   */
  async search(searchTerm: string, limit = 20): Promise<Cliente[]> {
    try {
      const query = `
   ...
   */
  async updateEstadisticas(clienteId: number): Promise<void> {
    try {
      const query = `
        UPDATE clientes 
        SET 
   ...
   */
  async getHistorialPedidos(clienteId: number, limit = 50): Promise<any[]> {
    try {
      const query = `
        SELECT 
          p.id,
   ...
   */
  async existsByTelefono(telefono: string, excludeId?: number): Promise<boolean> {
    try {
      let query = 'SELECT id FROM clientes WHERE telefono = $1';
      const values: any[] = [telefono];
