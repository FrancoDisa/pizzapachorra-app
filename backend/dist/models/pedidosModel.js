...
export class PedidosModel {
     ...
     */
    async getAll(estado, fechaInicio, fechaFin, clienteId, limit = 50, offset = 0) {
        try {
            let query = `
        SELECT 
          p.*,
     ...
     * Crear nuevo pedido
     */
    async create(pedidoData, numeroPedido) {
      ...
      `;
        ...
        `;
            ...
            await this.createHistorialEstado(client, nuevoPedido.id, undefined, 'nuevo', 'Pedido creado', 'sistema');
            await client.query('COMMIT');
            // Retornar pedido completo
            return await this.getById(nuevoPedido.id);
        }
        catch (error) {
     ...
     */
    async cambiarEstado(id, nuevoEstado, motivo, usuario = 'sistema') {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
      ...
      `;
            ...
            await this.createHistorialEstado(client, id, estadoAnterior, nuevoEstado, motivo, usuario);
            await client.query('COMMIT');
            return await this.getById(id);
        }
        catch (error) {
     ...
     */
    async getByEstado(estado, limit = 50) {
     ...
     */
    async getHistorialEstados(pedidoId) {
        try {
            const query = `
     ...
     */
    async buildPedidoCompleto(pedidoRow) {
     ...
     */
    async buildPedidoItemCompleto(itemRow) {
     ...
     * Crear registro en historial de estados
     */
    async createHistorialEstado(client, pedidoId, estadoAnterior, estadoNuevo, motivo, usuario = 'sistema') {
        const query = `
